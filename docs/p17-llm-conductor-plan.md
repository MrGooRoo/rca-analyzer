...
### 12.5. Этап 5 — Standalone LLMConductor (17.06.2026)

Добавлено:

- `src/services/llm_conductor.py` — standalone-сервис дирижирования моделей:
  1. вызывает `draft_model` с обычным prompt методологии;
  2. парсит draft через существующий `MethodologyRunner`;
  3. применяет gate по `verification_scheme` и `quality_threshold`;
  4. при необходимости вызывает `verifier_model` с `configs/prompts/verifier.j2`;
  5. парсит verified raw через тот же runner;
  6. объединяет audit-поля `model_used = "draft -> verifier"` и `tokens_used = draft + verifier`.
- `tests/unit/test_llm_conductor.py` — 4 unit-теста без реальных LLM:
  - `disabled` использует только draft;
  - `threshold` пропускает verifier при высокой уверенности;
  - `threshold` вызывает verifier при низкой уверенности и передаёт draft/low-confidence nodes;
  - `always` вызывает verifier независимо от confidence.

Проверки:

```text
ruff check src/services/llm_conductor.py tests/unit/test_llm_conductor.py → All checks passed!
pytest tests/unit/test_llm_conductor.py -q → 4 passed
python -m pytest tests/ -q → 284 passed, 1 deselected
```
