...
- Форма поддерживает смешанный сценарий ввода: пользователь может заполнить поля вручную, загрузить DOCX, а затем дозаполнить отсутствующие данные вручную. При upload заполненные вручную значения не затираются пустыми значениями из файла.
- Черновик формы сохраняется в `sessionStorage` под ключом `rca-analyzer.incident-form-draft.v1`, чтобы данные не терялись при переходе «Анализ ↔ История» в рамках текущей вкладки браузера.
- Порядок методик в форме: `five_why`, `bowtie`, `ishikawa`, `fta`, `rca_systemic`; методика по умолчанию — `five_why`.
