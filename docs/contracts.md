...
>
> **Дата актуализации:** 2026-06-14 (UI-состояния анализа и IncidentForm на UI-kit поля, см. разделы 10.4, 10.4.1, 16; ранее — сущность «исследование» analysis_session, разделы 10.5, 15).
...
- Для режима «Сравнить методики» frontend использует `api.analyzeMultiStream(payload, onEvent)`, а не обычный `api.analyzeMulti(payload)`.
...
- `AnalysisProgress.jsx` запускает `POST /api/v1/analyze-multi-stream` при монтировании и обрабатывает события:
  - `started` → создаёт список методик;
  - `progress` → отмечает методику как `done`;
  - `error_one` → отмечает отдельную методику как `error`;
