...
- Валидация JSON-ответа по формату contracts.md раздел 6
...
# Ключи, которые должны присутствовать в ответе LLM (contracts.md раздел 6)
...
class OpenRouterClient:
    ...
    async def complete(
        ...
        *,
        ...
        Returns:
            dict — разобранный JSON из ответа LLM + ключ _meta.
            Формат описан в contracts.md раздел 6.

        Raises:
            LLMResponseValidationError: если JSON невалиден или отсутствуют
                                        обязательные ключи после max_retries.
        """
        try:
    ...
    def _retry_decorator(self):
        """Декоратор retry — создаётся динамически, чтобы учесть max_retries."""
        return retry(
    ...
    async def _call_api(
        ...
        *,
        ...
        logger.info(
            "[OpenRouter] ← model=%s tokens=%d confidence_keys=%s",
            self.model,
            parsed["_meta"]["tokens"],
            list(parsed.keys()),
        )
    ...
    def _parse_and_validate(self, content: str) -> dict:
        """
        Разобрать JSON из строки и проверить наличие обязательных ключей.

        Raises:
            LLMResponseValidationError: если контент не является валидным JSON
                                        или отсутствуют обязательные ключи.
        """
        try:
            parsed: dict = json.loads(content)
        except json.JSONDecodeError as exc:
            logger.warning("[OpenRouter] Невалидный JSON: %s...", content[:200])
            raise LLMResponseValidationError(
