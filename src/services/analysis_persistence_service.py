...
class AnalysisPersistenceService:
    ...
    def __init__(
        self,
        service_getter: collections.abc.Callable[[], AnalysisService] | None = None,
    ) -> None:
    ...
    async def stream_single(
        ...
        if result is None:
            yield {"status": "error", "message": "Анализ не вернул результата."}
            return
        ...
        try:
            ...
            yield {"status": "error", "message": "Не удалось сохранить результат анализа."}
            return

        result.user_id = user_id
        yield {"status": "done", "result": result.model_dump(mode="json")}
